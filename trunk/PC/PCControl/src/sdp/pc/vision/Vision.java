...
public class Vision extends WindowAdapter implements CaptureCallback {
		...
		for (int row = leftTop.getY(); row < rightBottom.getY(); row++) {
			for (int column = leftTop.getX(); column < rightBottom.getX(); column++) {
				...
				if (Alg.isInHull(borders, p)) {
					pitchPoints.add(p);
					hullPoints.put(p.getX() * 1000 + p.getY(), 0);
	...
	private void processImage(BufferedImage image) {
		...
		avgPrevPos = avgPrevPos.subtract(ballPos).mult(-100 / (int) fps)
				.add(ballPos);
...
//				imageGraphics, Constants.ROBOT_YELLOW_LEFT);
		...
		if (Alg.pointInPitch(ballPos)) {
			...
			if (state.getBallVelocity().modulus() > 10) {
		...
		fps = Math.max(1.0f, (1.0f) / ((after - initialTime) / 1000.0f));
		imageGraphics.setColor(Color.white);
...
//	}
	 ...
	 */
	public static void drawCircle(Point2 centrePt, Graphics gfx, Color c, int radius) {
		gfx.setColor(c);
