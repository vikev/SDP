...
/**
 * Takes care of painting a {@link WorldState} overlay atop a {@link BufferedImage}. 
 * Supports highlighting specified colour ranges as defined in Colors.java
 * (incurring visible FPS drop on DICE machines).
 * 
 * @author Ix
 */
public class WorldStatePainter {
	/**
	 * The minimum speed considered as moving
	...
	/**
	 * The state listener we're attached to so we can use its normalised RGB/HSB data
	 */
	...
	/**
	 * Constructs a new WorldStatePainter given a WorldState and WorldStateListener to attach to
	 * @param stateListener the WorldStateListener to use for getting last frame's RGB/HSB data
	 * @param state the WorldState to attach and draw to
	 */
	 ...
	 * New values should have appropriate code attached to the getHighlight() function
	 * @author Ix
	 */
	enum HighlightMode {
	...
	/**
	 * Gets the colour that should be drawn on the image for the given RGB/HSB pair
	 * according to the current highlighting mode
	 * @param cRgb the RGB colour object of the pixel
	 * @param cHsb the HSB colour values of the pixel
	...
	/**
	 * Draws the world state overlay on the specified image 
	 * using this.state and this.stateListener. 
	 * <p>
	 * Currently implements: 
	 * <li>White highlighting on missing boundary;
	 * otherwise highlights according to highlightMode
	 * <li>draws the ball and its velocity, 
	 * all robots and their facing
	 * <li>overlay text showing last rendered image's (normalised) RGB/HSB values, 
	 * mouse position, current FPS
	 * <li>something else?
	 * @param image the image to draw the overlay on
	 */
	public void drawWorld(BufferedImage image, Point2 mousePos) {
		...
		//if pre-processing is not done yet just colour all 'white' pixels and return
...
//		angSmoothingWriteIndex %= ANGLE_SMOOTHING_FRAME_COUNT;
	 ...
	 * @param c the colour of the drawn circle
	 * @param centrePt the centre of the circle
	...
	/**
	 * Sets the custom highlight mode overlaid on colours of interest;
	 * has no effect if pitch boundaries are unavailable as the overlay will be HighlightMode.White
	 */
	...
	/**
	 * Gets the custom highlight mode for colours of interest;
	 * unspecified output if pitch boundaries are unavailable as the overlay will be HighlightMode.White
	 */
