...
public class Milestone3att {
	 ...
	 * discern target goal
	 * 	assign target point within goal (currently only use goal centre) - separate method
	 * 	create vectors for line ball to goal and for ball to robot using ball position as (0,0) point of coordinate axis
	 * 	assign ball's approach point
	 * 	if ball's approach point lies within selected robots bounding box then
	 ...
	 * 			face target goal and approach and kick ball 
	 * 		else 
	 * 			set way-point 90 degrees or 270 degrees to the line connecting ball to goal (whichever is closer to the angle between line connecting ball to the target goal and the line connecting the ball to the Robot)
	 * 			if way-point is within robots bounding box 
	 * 				go to way-point
	 * 				face and move to the ball's approach point
	 * 				face target goal and approach and kick ball 
	 * 			else
	 * 				Robot is stuck in a corner of its bounding box and can't kick the ball towards the goal.
	 * 				display that shot has been deemed impossible
	 * 	else
	 * 		display that shot has been deemed impossible
	 * 	end
	 * 
	 * Throughout this loop if ball location is changed then loop should start from the top again after a period of waiting for the ball's reported position to stabilise
	 * */
	
	public static void kickball (WorldState state, TCPClient robotControl, Vision vision, int chosenRobot){
		...
		if (targetPoint.getX() == 0 & targetPoint.getY() == 0){
			...
			return;		//could not set target point for ball and therefore cannot take shot
		}
		int[] vectorLineBalltoTargetPoint = {(state.getBallPosition().getX() - targetPoint.getX()), (state.getBallPosition().getY() - targetPoint.getY())};
		int[] vectorLineBalltoSelectedRobot = {(state.getBallPosition().getX() - state.getRobotPosition(0, chosenRobot).getX()), (state.getBallPosition().getY() - state.getRobotPosition(0, chosenRobot).getY())};
