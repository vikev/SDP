 ...
 * also the actual code processing the frame and updating the underlying world state
 * <p>
...
public class WorldStateUpdater extends WorldStateListener {
	...
	//contain [team][robot]
	private int[][] robotPtsCount = new int[2][2];	//the amount of points observed for each robot
	private Point2[][] robotPos = new Point2[2][2];	//the sum (later average) of the observed points
	@SuppressWarnings("unchecked")	//can't create generic arrays
	private ArrayList<Point2>[][] robotPts = new ArrayList[][] {	//the list of points for each robot
		new ArrayList[] {
	...
	/**
	 * Constructs a new WorldStateUpdater to look for new frames, as refreshed by setCurrentFrame().
	 * When a new frame is detected it first grabs the frame data using processImage() 
	 * and then uses updateWorld() to interpret it and update the specified {@link WorldState}
	 * @param targetFps the maximum FPS
	 * @param state the WorldState to be updated
	 */
	...
	/**
	 * Does an update to the underlying {@link WorldState} trying to fetch fresh values
	 * for the ball position and velocity, and the robots' position and facing. 
	 */
	@Override
